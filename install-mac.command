...
if [ -z "$ACTION" ]; then
  echo "请选择操作："
  echo "  [1] 安装中文补丁（完整模式：深度汉化，包含在线聊天网页 DOM，支持官方与第三方API。注意：会破坏签名，导致官方沙箱/工作区不可用）"
  echo "  [2] 安装中文补丁（安全模式：仅汉化本地菜单与设置，不破坏签名，官方沙箱功能正常。注意：不汉化网页聊天区，第三方API需搭配 CC Switch 使用）"
  echo "  [3] 恢复原样 / 卸载补丁"
