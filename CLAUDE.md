...
### 4n — Windows 模式 2 (官方模式) under PS5.1 JSON 大小写敏感键解析崩溃修复
- **发现**：在 Windows 上选择模式 2（官方模式）时，脚本在第 7 步 `Patch-OnlineDomTranslation` 提取在线 DOM 翻译字典时会调用 `ConvertFrom-Json`。而在 PowerShell 5.1 中，`ConvertFrom-Json` 将 JSON 键值对解析并挂载到大小写不敏感的 `PSCustomObject` 属性上。因为 Anthropic 最新的 `en-US.json` 与 `zh-CN.json` 包含了仅有大小写差异的唯一 ID 键（例如 `/AyXW9R0LE` 与 `/AyXW9R0Le`），引发 `ConvertFrom-Json` 抛出 `字典包含重复的键` 的终止性错误，使安装进程崩溃中断。
- **修复**：将 `Get-OnlineTranslationMap` 内的 JSON 解析引擎重构为使用 .NET 的 `JavaScriptSerializer`。由于 .NET Dictionary 本身是大小写敏感的，完美避开了 PSCustomObject 的键命名限制。同时将原先低效的 `$en.PSObject.Properties` 循环查表结构重构为直接遍历 `$en.Keys` 的字典哈希互查（`O(1)`），在解决崩溃的同时大幅提升了 Asar 处理阶段的打包性能。
